...
module.exports = {
    ...
    LIST: async function (data, callback) {
        ...
        try {
            ...
            const numRows = await OnlineModel.countDocuments(condition);
            const records = await OnlineModel.find(condition).skip((start - 1) * limit).limit(limit);
    ...
    VIEW: async function (data, callback) {
        ...
        try {
            ...
            const record = await OnlineModel.findOne(condition);
            if (record) {
    ...
    DELETE: async function (data, callback) {
        ...
        try {
            ...
            if (id) {
                ...
                const resp = await OnlineModel.updateMany(condition, { deleted: true });
                console.log("resp: ", resp);
                sendData.data = resp;
                sendData.msg = "Online transaction deleted successfully";
            } else {
                sendData.err = 1;
                sendData.msg = "No record found!";
            }
